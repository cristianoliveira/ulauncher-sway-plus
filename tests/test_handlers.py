...
class TestGetResultItem(TestCase):
    def test_get_result_item_basic(self):
        ...
        with (
            mock.patch(
                "handlers.windows.sway_icons.get_icon", return_value="icon-path"
            ) as mock_get_icon,
            mock.patch(
                "handlers.windows.windows.app_details",
                return_value=("1", "FakeApp", "Fake Window"),
            ) as mock_app_details,
            mock.patch("handlers.windows.ExtensionResultItem") as mock_ext_result_item,
            mock.patch(
                "handlers.windows.ExtensionCustomAction"
            ) as mock_ext_custom_action,
        ):
